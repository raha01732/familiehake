...
const fakeStatus = [
  ...
  { name: "Fehlversuche Login", state: "LOW", detail: "1 fehlgeschlagener Login (24h)" }
];
      ...
      <section className="grid gap-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="card p-6 flex flex-col gap-4">
            ...
            <div className="text-[11px] text-zinc-600">Stand: 29.10.2025</div>
          </div>
        ...
        {/* Event-Log (bald echt via Audit-Logs) */}
        <div className="card p-6 flex flex-col gap-4">
          <div>
            <h2 className="text-xl font-semibold text-zinc-100 tracking-tight">Letzte Ereignisse</h2>
            <p className="text-zinc-400 text-sm leading-relaxed">Jüngste sicherheitsrelevante Aktionen.</p>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-zinc-800">
                {fakeEvents.map((row, idx) => (
                  <tr key={idx}>
                    <td className="px-3 py-2 text-zinc-300 text-xs whitespace-nowrap">{row.ts}</td>
                    <td className="px-3 py-2 text-zinc-400 text-xs">{row.msg}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-[11px] text-zinc-600">
            Hinweis: Aktuell statisch. In Schritt (2) ersetzen wir das durch echte Audit-Logs aus der DB.
          </div>
        </div>
      </section>
    </RoleGate>
