...
export default function MitarbeiterClient({
  ...
  // undefined = closed, null = new, Employee = edit
...
function EmployeeCard({ employee: emp, onEdit, muted }: {
  ...
  return (
    <div
      ...
      onClick={() => onEdit(emp)}
    >
      <div className="absolute top-0 left-0 right-0 h-1 rounded-t-xl" style={{ backgroundColor: emp.color }} />
      <div className="flex items-start gap-3 mt-1">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
            {emp.position && <span className="text-xs text-zinc-400 truncate">{emp.position}</span>}
            {emp.department && emp.position && <span className="text-zinc-700 text-xs">·</span>}
            {emp.department && <span className="text-xs text-zinc-500 truncate">{emp.department}</span>}
          </div>
        </div>
        <svg className="w-4 h-4 text-zinc-600 group-hover:text-zinc-400 transition-colors flex-shrink-0 mt-0.5"
      ...
      <div className="mt-3 flex items-center justify-between text-xs text-zinc-500">
        <div className="flex items-center gap-3">
          {emp.monthly_hours > 0 && (
            <span><span className="text-zinc-300 font-medium">{emp.monthly_hours}h</span><span className="text-zinc-600">/Monat</span></span>
          )}
          {emp.weekly_hours > 0 && (
            <span><span className="text-zinc-300 font-medium">{emp.weekly_hours}h</span><span className="text-zinc-600">/Woche</span></span>
          )}
        </div>
        <span className="px-2 py-0.5 bg-zinc-800 rounded-full text-zinc-400 text-[10px]">{empType}</span>
      </div>
    </div>
  );
}
