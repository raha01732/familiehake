...
import { bulkSaveShiftsAction, clearMonthAction, saveShiftAction } from "./actions";
import {
    ...
    <section className="p-6 flex flex-col gap-6">
      <header className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link
            href="/tools/dienstplaner/settings"
            ...
            {days.map((day) => {
              ...
              return (
                <tr key={dateKey} className="border-t border-zinc-800 align-top">
                  <td className="py-3 px-4 text-zinc-300 whitespace-nowrap">{formatDateLabel(day)}</td>
                  {(employees as DienstplanEmployee[] | null)?.map((employee) => {
      ...
      {((employees as DienstplanEmployee[] | null) ?? []).length === 0 && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6 text-sm text-zinc-300">
          Noch keine Mitarbeitenden angelegt. Lege sie in den <Link href="/tools/dienstplaner/settings" className="underline">Einstellungen</Link> an.
        </div>
      )}
    </section>
  );
