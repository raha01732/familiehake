...
const CLERK_CACHE_KEY = "monitoring:clerk:summary:v2";
...
export async function fetchClerkStats(): Promise<ClerkStats> {
  ...
  try {
    ...
    const [totalUsersRes, activeUsersRes, invitationsRes] = await Promise.all([
      ...
      fetchFromClerk<ClerkPaginated<unknown>>(
        `/users?last_active_at_since=${since}&limit=1`
      ),
    ...
    const activeUsers24h = activeUsersRes?.total_count ?? 0;
